...
void gravaFicheiro(int *nrFicheiro, int *vecNumeros, int lo, int hi, int inicio, int fim) {
    ...
    int count1 = count + 1;
    *nrFicheiro = count1;
    char vec1[] = "/home/rui/Documentos/projectofinao/projecto/dados/";
    char vec2[] = ".txt";
...
int *carregarFicheiro(int *vetor) {
    ...
    char buf[1];
    int count = 0;
    //if ((fd = open("/home/rui/Desktop/apresentacao/ponto4/dados.txt", O_CREAT | O_RDWR, 0644)) < 0) {
    if ((fd = open("/home/rui/Desktop/apresentacao/ponto4/dados1.txt", O_CREAT | O_RDWR, 0644)) < 0) {
...
void cria_ficheiro_de_numeros(int numero_dados) {
    FILE *ficheiro;
    if ((ficheiro = fopen("/home/rui/Desktop/apresentacao/ponto4/dados1.txt", "w")) == NULL) {
    ...
    for (i = 0; i < numero_dados; i++) {
        numero = rand() % numero_dados;
        fprintf(ficheiro, "%d", numero);
        if (i == corte) {
            fprintf(ficheiro, ";\n");
            corte += 10;
        } else {
